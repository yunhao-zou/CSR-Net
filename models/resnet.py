...
class ResNet(nn.Module):
    ...
    def __init__(self, block, layers, num_classes=1000, dilated=True, norm_layer=nn.BatchNorm2d, multi_grid=False, multi_dilation=None, crf=False, crf_channel=200, n_bands=3):
        self.crf = crf
        self.crf_channel = crf_channel
        self.inplanes = 64
        super(ResNet, self).__init__()
        self.filter = nn.Conv2d(n_bands, crf_channel, kernel_size=1, bias=False)
        self.conv1 = nn.Conv2d(crf_channel if self.crf else n_bands, 64, kernel_size=7, stride=2, padding=3,
                               bias=False)
